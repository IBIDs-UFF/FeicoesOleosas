from oil import BACKGROUND_ID, BackgroundSubtractor, DropTracker
from tqdm import tqdm
from typing import List, NamedTuple
import cv2
import gc, glob, os
import matplotlib.pyplot as plt
import numpy as np
import oil


DATASET_DIR = os.path.join('..', 'FeicoesOleosas-Dataset', 'Petrobras')

DEFAULT_PATIENCE_IN_SECONDS = 2
BACKGROUND_LEARNING_IN_SECONDS = 5
SKIP_SECONDS = 0

UNKNOWN_ID = -1

BACKGROUND_ID_COLOR = np.asarray((255, 255, 255), dtype=np.uint8)
UNKNOWN_ID_COLOR = np.asarray((0, 0, 0), dtype=np.uint8)
DROP_ID_CMAP = (np.asarray(plt.cm.tab20.colors) * 255).astype(np.uint8)


class BufferItem(NamedTuple):
    frame_rgb: np.ndarray
    frame_gray: np.ndarray
    frame_flow: np.ndarray
    is_background: np.ndarray

#TODO
# def compute_flow(frame_rgb: np.ndarray, frame_gray: np.ndarray, buffer: List[BufferItem], buffer_ind: int) -> np.ndarray:
#     frame_flow = cv2.optflow.calcOpticalFlowSparseToDense(frame_rgb, buffer[(buffer_ind - 1) % len(buffer)].frame_rgb, None, 2)
#     frame_flow += 0.5
#     return frame_flow

#TODO
def compute_flow(frame_rgb: np.ndarray, frame_gray: np.ndarray, buffer: List[BufferItem], buffer_ind: int) -> np.ndarray:
    frame_flow = cv2.calcOpticalFlowFarneback(frame_gray, buffer[(buffer_ind - 1) % len(buffer)].frame_gray, None, 0.5, 3, 15, 3, 5, 1.2, 0)
    frame_flow += 0.5
    return frame_flow


def main() -> None:
    scale_factor = 0.25
    # Set style.
    plt.rc('axes', titlesize=5, labelsize=2)
    plt.rc('xtick', labelsize=2)
    plt.rc('ytick', labelsize=2)
    plt.rc('lines', linewidth=0.1, markersize=0.5)
    plt.rc('grid', color='0.5', linestyle='-', linewidth=0.1)
    # For each input video...
    for video_path in sorted(glob.glob(os.path.join(DATASET_DIR, '*.mp4'))):
        basename = os.path.basename(video_path)
        # Create the video capture object and open the input file.
        with oil.VideoReader(video_path) as video:
            pbar = iter(tqdm(video, desc=f'Processing "{basename}"', total=video.frame_count))
            # Initialize some useful variables.
            frame_ind = 0
            used_width = int(video.width * scale_factor)
            used_height = int(video.height * scale_factor)
            time_series = np.arange(video.frame_count, dtype=np.float32) / video.fps
            area_counts = {}
            pixel_ind = np.stack(np.meshgrid(np.arange(used_width, dtype=np.int32), np.arange(used_height, dtype=np.int32), indexing='xy'), axis=2)
            # Create and train the background subtractor.
            subtractor = BackgroundSubtractor()
            num_background_samples = BACKGROUND_LEARNING_IN_SECONDS * video.fps
            background_sample_lab = np.zeros((num_background_samples, used_height, used_width, 3), dtype=np.uint8)
            for ind in range(num_background_samples):
                background_sample_lab[ind, ...] = cv2.cvtColor(cv2.resize(next(pbar), (used_width, used_height)), cv2.COLOR_RGB2LAB)
                frame_ind += 1
            subtractor.train(background_sample_lab)
            # Create a drop tracker.
            patience = int(DEFAULT_PATIENCE_IN_SECONDS * video.fps)
            tracker = DropTracker(subtractor, patience=patience)
            # Create tracking buffer.
            buffer: List[BufferItem] = [None] * patience
            # Skip the first part of boring videos (for debug only).
            num_boring_frames = SKIP_SECONDS * video.fps
            for _ in range(max(num_boring_frames - num_background_samples, 0)):
                next(pbar)
                frame_ind += 1
            # Fill the buffer.
            frame_rgb = cv2.resize(next(pbar), (used_width, used_height))
            frame_gray = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2GRAY)
            frame_ind += 1
            buffer[0] = BufferItem(frame_rgb, frame_gray, np.zeros((used_height, used_width, 2), dtype=np.float32), np.full((used_height, used_width), True, dtype=np.bool_))
            buffer_ind = 1
            for _ in range(buffer_ind, len(buffer)):
                frame_rgb = cv2.resize(next(pbar), (used_width, used_height))
                frame_gray = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2GRAY)
                frame_flow = compute_flow(frame_rgb, frame_gray, buffer, buffer_ind)
                tracker.update(frame_rgb, frame_gray, frame_ind)
                assert tracker.lost_drops_footprint is None
                buffer[buffer_ind] = BufferItem(frame_rgb, frame_gray, frame_flow, tracker.background_msk.astype(np.bool_))
                frame_ind += 1
                buffer_ind += 1
            # Create the video output video.
            with oil.VideoWriter(f'{os.path.splitext(basename)[0]}-flow.mp4', (4 * used_width, used_height), video.fps) as out:
                oil_spill = np.full((used_height, used_width), BACKGROUND_ID, dtype=np.int32)
                for _ in range(frame_ind, video.frame_count):
                    # Update tracker.
                    frame_rgb = cv2.resize(next(pbar), (used_width, used_height))
                    frame_gray = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2GRAY)
                    tracker.update(frame_rgb, frame_gray, frame_ind)
                    # Compute flow.
                    frame_flow = compute_flow(frame_rgb, frame_gray, buffer, buffer_ind)
                    # Draw active drops.
                    for drop in tracker.drops:
                        if drop['active']:
                            x, y, w, h = map(int, drop['bbox'])
                            color = (255, 0, 255) if drop['last_seen'] == frame_ind else (127, 127, 127)
                            cv2.rectangle(frame_rgb, (x, y), (x + w, y + h), color, 1)
                    # Draw lost drops
                    if tracker.lost_drops_footprint is not None:
                        is_lost_drop = tracker.lost_drops_footprint != BACKGROUND_ID
                        oil_spill[is_lost_drop] = tracker.lost_drops_footprint[is_lost_drop]
                    # # Follow the oil spill.
                    # spill_from = (buffer[buffer_ind % len(buffer)].frame_flow + pixel_ind).astype(np.int32)
                    # from_x = np.clip(spill_from[..., 0].ravel(), 0, used_width - 1)
                    # from_y = np.clip(spill_from[..., 1].ravel(), 0, used_height - 1)
                    # new_oil_spill = oil_spill[from_y, from_x].reshape(used_height, used_width)
                    # is_oil = new_oil_spill > max(UNKNOWN_ID, BACKGROUND_ID)
                    # oil_spill[is_oil] = new_oil_spill[is_oil]
                    # oil_spill_rgb = np.full((used_height, used_width, 3), BACKGROUND_ID_COLOR, dtype=np.uint8)
                    # is_oil = oil_spill > max(UNKNOWN_ID, BACKGROUND_ID)
                    # oil_spill_rgb[is_oil, :] = DROP_ID_CMAP[oil_spill[is_oil].ravel() % len(DROP_ID_CMAP), :]
                    # Follow the oil spill.
                    spill_from = (buffer[buffer_ind % len(buffer)].frame_flow + pixel_ind).astype(np.int32)
                    from_x = np.clip(spill_from[..., 0].ravel(), 0, used_width - 1)
                    from_y = np.clip(spill_from[..., 1].ravel(), 0, used_height - 1)
                    new_oil_spill = oil_spill[from_y, from_x].reshape(used_height, used_width)
                    oil_spill = np.maximum(oil_spill, new_oil_spill)
                    oil_spill_rgb = np.full((used_height, used_width, 3), BACKGROUND_ID_COLOR, dtype=np.uint8)
                    is_oil = oil_spill > max(UNKNOWN_ID, BACKGROUND_ID)
                    oil_spill_rgb[is_oil, :] = DROP_ID_CMAP[oil_spill[is_oil].ravel() % len(DROP_ID_CMAP), :]
                    # Update visible background statistics
                    drop_ids, spill_areas = np.unique(oil_spill[is_oil], return_counts=True)
                    for drop_id, spill_area in zip(drop_ids, spill_areas):
                        area_series = area_counts.get(drop_id, None)
                        if area_series is None:
                            area_series = np.zeros((video.frame_count,), dtype=np.float32)
                            area_counts[drop_id] = area_series
                        area_series[frame_ind - patience] = spill_area * (100 / (used_width * used_height))
                    # Plot data.
                    fig, (ax_area, ax_drop) = plt.subplots(2, 1, dpi=300, figsize=(2 * used_width / 300, used_height / 300))
                    try:
                        # Area series.
                        if len(area_counts) > 0:
                            series = []
                            colors = []
                            for drop_id, area_series in area_counts.items():
                                series.append(area_series)
                                color = DROP_ID_CMAP[drop_id % len(DROP_ID_CMAP), :]
                                colors.append(f'#{color[0]:02x}{color[1]:02x}{color[2]:02x}')
                            ax_area.stackplot(time_series, *series, colors=colors)
                        ax_area.set_title('Areas')
                        ax_area.axes.set_xlabel('Time (seconds)')
                        ax_area.axes.set_ylabel('Area (%)')
                        ax_area.axes.set_ylim(0.0, 105.0)
                        ax_area.grid(True)
                        # Drop.
                        max_id = 0
                        for drop in tracker.drops:
                            id = drop['id']
                            plt.plot((time_series[drop['first_seen']], time_series[drop['last_seen']]), (id, id), marker='x', color=DROP_ID_CMAP[id % len(DROP_ID_CMAP), :] / 256)
                            max_id = max(max_id, id)
                        ax_drop.set_title('Detected Drops')
                        ax_drop.axes.set_xlabel('Time (seconds)')
                        ax_drop.axes.set_ylabel('Drop')
                        ax_drop.axes.set_ylim(0, max_id + 1)
                        ax_drop.axes.set_yticks(np.arange(max_id + 1))
                        ax_drop.axes.set_yticklabels(['', *map(str, range(1, max_id + 1))])
                        ax_drop.grid(True)
                        # Draw plots.
                        plt.tight_layout()
                        fig.canvas.draw()
                        analysis_rgb = np.array(fig.canvas.renderer.buffer_rgba())[:, :, :3]
                    finally:
                        plt.close(fig)
                        gc.collect()
                    # Write data.
                    out.write(np.concatenate((buffer[buffer_ind % len(buffer)].frame_rgb, oil_spill_rgb, analysis_rgb), axis=1))
                    # Update buffer.
                    buffer[buffer_ind % len(buffer)] = BufferItem(frame_rgb, frame_gray, frame_flow, tracker.background_msk.astype(np.bool_))
                    frame_ind += 1
                    buffer_ind += 1


if __name__ == '__main__':
    main()
